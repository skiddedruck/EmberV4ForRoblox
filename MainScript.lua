...
local function vapeGithubRequest(scripturl)
	if not isfile("vapev4ember/"..scripturl) then
		...
		suc, res = pcall(function() return game:HttpGet("https://raw.githubusercontent.com/skiddedruck/EmberV4ForRoblox/"..readfile("vapev4ember/commithash.txt").."/"..scripturl, true) end)
		if not suc or res == "404: Not Found" then
			displayErrorPopup("Failed to connect to github : vapev4ember/"..scripturl.." : "..res)
			error(res)
		end
...
for i,v in pairs({baseDirectory:gsub("/", ""), "vapev4ember", "vapev4ember/Libraries", "vapev4ember/CustomModules", "vapev4ember/Profiles", baseDirectory.."Profiles", "vapev4ember/assets"}) do
	if not isfolder(v) then makefolder(v) end
end
task.spawn(function()
	local success, assetver = pcall(function() return vapeGithubRequest("assetsversion.txt") end)
	if not isfile("vapev4ember/assetsversion.txt") then writefile("vapev4ember/assetsversion.txt", "0") end
	if success and assetver > readfile("vapev4ember/assetsversion.txt") then
		redownloadedAssets = true
		if isfolder("vapev4ember/assets") and not shared.VapeDeveloper then
			if delfolder then
				delfolder("vapev4ember/assets")
				makefolder("vapev4ember/assets")
			end
		end
		writefile("vapev4ember/assetsversion.txt", assetver)
	end
end)
if not isfile("vapev4ember/CustomModules/cachechecked.txt") then
	local isNotCached = false
	for i,v in pairs({"vapev4ember/Universal.lua", "vapev4ember/MainScript.lua", "vapev4ember/GuiLibrary.lua"}) do
	...
	if isfolder("vapev4ember/CustomModules") then
		for i,v in pairs(listfiles("vapev4ember/CustomModules")) do
			if isfile(v) and not readfile(v):find("--This watermark is used to delete the file if its cached, remove it to make the file persist after commits.") then
				isNotCached = true
			end
		end
		...
		displayErrorPopup("Vape has detected uncached files, If you have CustomModules click no, else click yes.", {No = function() end, Yes = function()
			...
			for i,v in pairs(listfiles("vapev4ember/CustomModules")) do
				if isfile(v) and not readfile(v):find("--This watermark is used to delete the file if its cached, remove it to make the file persist after commits.") then
					local last = v:split('\\')
					last = last[#last]
					local suc, publicrepo = pcall(function() return game:HttpGet("https://raw.githubusercontent.com/skiddedruck/EmberV4ForRoblox/"..readfile("vapev4ember/commithash.txt").."/CustomModules/"..last) end)
					if suc and publicrepo and publicrepo ~= "404: Not Found" then
						writefile("vapev4ember/CustomModules/"..last, "--This watermark is used to delete the file if its cached, remove it to make the file persist after commits.\n"..publicrepo)
					end
				end
			end
		end})
	end
	writefile("vapev4ember/CustomModules/cachechecked.txt", "verified")
end
...
task.spawn(function()
	...
	image.Image = downloadVapeAsset("vapev4ember/assets/CombatIcon.png")
	...
	task.spawn(function()
		task.wait(15)
		if image and image.ContentImageSize == Vector2.zero and (not errorPopupShown) and (not redownloadedAssets) and (not isfile("vapev4ember/assets/check3.txt")) then
            errorPopupShown = true
            displayErrorPopup("Assets failed to load, Try another executor (executor : "..(identifyexecutor and identifyexecutor() or "Unknown")..")", {OK = function()
                writefile("vapev4ember/assets/check3.txt", "")
            end})
        end
	end)
end)
...
local Combat = GuiLibrary.CreateWindow({
	Name = "Combat",
	Icon = "vapev4ember/assets/CombatIcon.png",
	IconSize = 15
})
local Blatant = GuiLibrary.CreateWindow({
	Name = "Blatant",
	Icon = "vapev4ember/assets/BlatantIcon.png",
	IconSize = 16
})
local Render = GuiLibrary.CreateWindow({
	Name = "Render",
	Icon = "vapev4ember/assets/RenderIcon.png",
	IconSize = 17
})
local Utility = GuiLibrary.CreateWindow({
	Name = "Utility",
	Icon = "vapev4ember/assets/UtilityIcon.png",
	IconSize = 17
})
local World = GuiLibrary.CreateWindow({
	Name = "World",
	Icon = "vapev4ember/assets/WorldIcon.png",
	IconSize = 16
})
local Friends = GuiLibrary.CreateWindow2({
	Name = "Friends",
	Icon = "vapev4ember/assets/FriendsIcon.png",
	IconSize = 17
})
...
local Profiles = GuiLibrary.CreateWindow2({
	Name = "Profiles",
	Icon = "vapev4ember/assets/ProfilesIcon.png",
	IconSize = 19
})
GUI.CreateDivider()
GUI.CreateButton({
...
OnlineProfilesButtonImage.Image = downloadVapeAsset("vapev4ember/assets/OnlineProfilesButton.png")
...
OnlineProfilesExitButton.Image = downloadVapeAsset("vapev4ember/assets/ExitIcon1.png")
...
OnlineProfilesFrameShadow.Image = downloadVapeAsset("vapev4ember/assets/WindowBlur.png")
...
OnlineProfilesFrameIcon.Image = downloadVapeAsset("vapev4ember/assets/ProfilesIcon.png")
...
local TextGUI = GuiLibrary.CreateCustomWindow({
	Name = "Text GUI",
	Icon = "vapev4ember/assets/TextGUIIcon1.png",
	IconSize = 21
})
local TextGUICircleObject = {CircleList = {}}
GUI.CreateCustomToggle({
	Name = "Text GUI",
	Icon = "vapev4ember/assets/TextGUIIcon3.png",
...
VapeLogo.Image = downloadVapeAsset("vapev4ember/assets/VapeLogo3.png")
...
VapeLogoV4.Image = downloadVapeAsset("vapev4ember/assets/VapeLogo4.png")
...
TextGUIMode = TextGUI.CreateDropdown({
	...
	Function = function(val)
		...
		if val == "Drawing" then
			...
			VapeLogoDrawing.Data = readfile("vapev4ember/assets/VapeLogo3.png")
			...
			VapeLogoV4Drawing.Data = readfile("vapev4ember/assets/VapeLogo4.png")
			...
			VapeLogoShadowDrawing.Data = readfile("vapev4ember/assets/VapeLogo3.png")
			...
			VapeLogo4Drawing.Data = readfile("vapev4ember/assets/VapeLogo4.png")
...
local TargetInfo = GuiLibrary.CreateCustomWindow({
	Name = "Target Info",
	Icon = "vapev4ember/assets/TargetInfoIcon1.png",
	IconSize = 16
})
...
TargetInfoHealthBackgroundShadow.Image = downloadVapeAsset("vapev4ember/assets/WindowBlur.png")
...
GUI.CreateCustomToggle({
	Name = "Target Info",
	Icon = "vapev4ember/assets/TargetInfoIcon2.png",
    ...
    if (not teleportedServers) and (not shared.VapeIndependent) then
		...
		local teleportScript = [[
			...
			if shared.VapeDeveloper then
				loadstring(readfile("vapev4ember/NewMainScript.lua"))()
			else
				loadstring(game:HttpGet("https://raw.githubusercontent.com/skiddedruck/EmberV4ForRoblox/"..readfile("vapev4ember/commithash.txt").."/NewMainScript.lua", true))()
			end
		]]
		if shared.VapeDeveloper then
...
local function loadVape()
	if not shared.VapeIndependent then
		...
		if isfile("vapev4ember/CustomModules/"..game.PlaceId..".lua") then
			loadstring(readfile("vapev4ember/CustomModules/"..game.PlaceId..".lua"))()
		else
			if not shared.VapeDeveloper then
				local suc, publicrepo = pcall(function() return game:HttpGet("https://raw.githubusercontent.com/skiddedruck/EmberV4ForRoblox/"..readfile("vapev4ember/commithash.txt").."/CustomModules/"..game.PlaceId..".lua") end)
				if suc and publicrepo and publicrepo ~= "404: Not Found" then
					writefile("vapev4ember/CustomModules/"..game.PlaceId..".lua", "--This watermark is used to delete the file if its cached, remove it to make the file persist after commits.\n"..publicrepo)
					loadstring(readfile("vapev4ember/CustomModules/"..game.PlaceId..".lua"))()
				end
			end
		end
