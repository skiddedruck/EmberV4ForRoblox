...
for i,v in pairs({baseDirectory:gsub("/", ""), "vape", "ember/Libraries", "ember/CustomModules", "ember/Profiles", baseDirectory.."Profiles", "ember/assets"}) do
	if not isfolder(v) then makefolder(v) end
end
task.spawn(function()
	local success, assetver = pcall(function() return vapeGithubRequest("assetsversion.txt") end)
	if not isfile("ember/assetsversion.txt") then writefile("ember/assetsversion.txt", "0") end
	if success and assetver > readfile("ember/assetsversion.txt") then
		redownloadedAssets = true
		if isfolder("ember/assets") and not shared.VapeDeveloper then
			if delfolder then
				delfolder("ember/assets")
				makefolder("ember/assets")
			end
		end
		writefile("ember/assetsversion.txt", assetver)
	end
end)
if not isfile("ember/CustomModules/cachechecked.txt") then
		...
		displayErrorPopup("Vape has detected uncached files, If you have CustomModules click no, else click yes.", {No = function() end, Yes = function()
			...
			for i,v in pairs(listfiles("ember/CustomModules")) do
				if isfile(v) and not readfile(v):find("--This watermark is used to delete the file if its cached, remove it to make the file persist after commits.") then
					local last = v:split('\\')
					last = last[#last]
					local suc, publicrepo = pcall(function() return game:HttpGet("https://raw.githubusercontent.com/skiddedruck/EmberV4ForRoblox/"..readfile("ember/commithash.txt").."/CustomModules/"..last) end)
					if suc and publicrepo and publicrepo ~= "404: Not Found" then
						writefile("ember/CustomModules/"..last, "--This watermark is used to delete the file if its cached, remove it to make the file persist after commits.\n"..publicrepo)
					end
				end
			end
		end})
	end
	writefile("ember/CustomModules/cachechecked.txt", "verified")
end
...
task.spawn(function()
	...
	image.Image = downloadVapeAsset("ember/assets/CombatIcon.png")
	...
	task.spawn(function()
		task.wait(15)
		if image and image.ContentImageSize == Vector2.zero and (not errorPopupShown) and (not redownloadedAssets) and (not isfile("ember/assets/check3.txt")) then
            errorPopupShown = true
            displayErrorPopup("Assets failed to load, Try another executor (executor : "..(identifyexecutor and identifyexecutor() or "Unknown")..")", {OK = function()
                writefile("ember/assets/check3.txt", "")
            end})
        end
	end)
end)
...
local Combat = GuiLibrary.CreateWindow({
	Name = "Combat",
	Icon = "ember/assets/CombatIcon.png",
	IconSize = 15
})
local Blatant = GuiLibrary.CreateWindow({
	Name = "Blatant",
	Icon = "ember/assets/BlatantIcon.png",
	IconSize = 16
})
local Render = GuiLibrary.CreateWindow({
	Name = "Render",
	Icon = "ember/assets/RenderIcon.png",
	IconSize = 17
})
local Utility = GuiLibrary.CreateWindow({
	Name = "Utility",
	Icon = "ember/assets/UtilityIcon.png",
	IconSize = 17
})
local World = GuiLibrary.CreateWindow({
	Name = "World",
	Icon = "ember/assets/WorldIcon.png",
	IconSize = 16
})
local Friends = GuiLibrary.CreateWindow2({
	Name = "Friends",
	Icon = "ember/assets/FriendsIcon.png",
	IconSize = 17
})
...
local Profiles = GuiLibrary.CreateWindow2({
	Name = "Profiles",
	Icon = "ember/assets/ProfilesIcon.png",
	IconSize = 19
})
GUI.CreateDivider()
GUI.CreateButton({
...
OnlineProfilesButtonImage.Image = downloadVapeAsset("ember/assets/OnlineProfilesButton.png")
...
OnlineProfilesExitButton.Image = downloadVapeAsset("ember/assets/ExitIcon1.png")
...
OnlineProfilesFrameShadow.Image = downloadVapeAsset("ember/assets/WindowBlur.png")
...
OnlineProfilesFrameIcon.Image = downloadVapeAsset("ember/assets/ProfilesIcon.png")
...
local TextGUI = GuiLibrary.CreateCustomWindow({
	Name = "Text GUI",
	Icon = "ember/assets/TextGUIIcon1.png",
	IconSize = 21
})
local TextGUICircleObject = {CircleList = {}}
GUI.CreateCustomToggle({
	Name = "Text GUI",
	Icon = "ember/assets/TextGUIIcon3.png",
...
VapeLogo.Image = downloadVapeAsset("ember/assets/VapeLogo3.png")
...
VapeLogoV4.Image = downloadVapeAsset("ember/assets/VapeLogo4.png")
...
TextGUIMode = TextGUI.CreateDropdown({
	...
	Function = function(val)
		...
		if val == "Drawing" then
			...
			VapeLogoDrawing.Data = readfile("ember/assets/VapeLogo3.png")
			...
			VapeLogoV4Drawing.Data = readfile("ember/assets/VapeLogo4.png")
			...
			VapeLogoShadowDrawing.Data = readfile("ember/assets/VapeLogo3.png")
			...
			VapeLogo4Drawing.Data = readfile("ember/assets/VapeLogo4.png")
...
local TargetInfo = GuiLibrary.CreateCustomWindow({
	Name = "Target Info",
	Icon = "ember/assets/TargetInfoIcon1.png",
	IconSize = 16
})
...
TargetInfoHealthBackgroundShadow.Image = downloadVapeAsset("ember/assets/WindowBlur.png")
...
GUI.CreateCustomToggle({
	Name = "Target Info",
	Icon = "ember/assets/TargetInfoIcon2.png",
    ...
    if (not teleportedServers) and (not shared.VapeIndependent) then
		...
		local teleportScript = [[
			...
			if shared.VapeDeveloper then
				loadstring(readfile("ember/NewMainScript.lua"))()
			else
				loadstring(game:HttpGet("https://raw.githubusercontent.com/skiddedruck/EmberV4ForRoblox/"..readfile("ember/commithash.txt").."/NewMainScript.lua", true))()
			end
		]]
		if shared.VapeDeveloper then
...
local function loadVape()
	if not shared.VapeIndependent then
		...
		if isfile("ember/CustomModules/"..game.PlaceId..".lua") then
			loadstring(readfile("ember/CustomModules/"..game.PlaceId..".lua"))()
		else
			if not shared.VapeDeveloper then
				local suc, publicrepo = pcall(function() return game:HttpGet("https://raw.githubusercontent.com/skiddedruck/EmberV4ForRoblox/"..readfile("ember/commithash.txt").."/CustomModules/"..game.PlaceId..".lua") end)
				if suc and publicrepo and publicrepo ~= "404: Not Found" then
					writefile("ember/CustomModules/"..game.PlaceId..".lua", "--This watermark is used to delete the file if its cached, remove it to make the file persist after commits.\n"..publicrepo)
					loadstring(readfile("ember/CustomModules/"..game.PlaceId..".lua"))()
				end
			end
		end
